...
export function HangarDetail({ kit }: { kit: Kit }) {
  ...
  return (
    <div className="container mx-auto px-6 py-10">
      ...
      <BlueprintFrame className="mt-6 p-6">
      ...
      <BlueprintFrame variant="magenta" className="mt-6 p-6">
        <h3 className="font-mono text-[11px] uppercase tracking-widest text-[var(--color-text-muted)]">
          {'// 我的状态'}
        </h3>
        <div className="mt-3">
          <StatusToggle
            ...
            onChange={(next) => setKitStatus(kit.id, next)}
          />
        </div>
        <NotesField
          kitId={kit.id}
          initial={hydrated ? (ownership?.notes ?? '') : ''}
          onSave={(notes) => setKitNotes(kit.id, notes)}
        />
      </BlueprintFrame>

      <BlueprintFrame variant="cyan" className="mt-6 p-6">
        <h3 className="font-mono text-sm uppercase tracking-wider text-[var(--color-text-muted)]">
          拼这台你需要 // recommended toolbox
        </h3>
